...
module CommandHelper
  ...
  def rhc_create_domain(app)
    rhc_do('rhc_create_domain') do
      ...
      time = Benchmark.realtime do 
        exit_code = run("#{$rhc_client} domain create #{app.namespace} -l #{app.login} -p #{app.password} -d")
      end
  ...
  def rhc_update_namespace(app)
    rhc_do('rhc_update_namespace') do
      ...
      time = Benchmark.realtime do 
        run("#{$rhc_client} domain update #{old_namespace} #{new_namespace} -l #{app.login} -p #{app.password} -d").should == 0
      end
  ...
  def rhc_snapshot(app)
    rhc_do('rhc_snapshot') do
      ...
      time = Benchmark.realtime do 
        run("#{$rhc_client} snapshot save #{app.name} -f '#{app.snapshot}' -l #{app.login} -p #{app.password} -d").should == 0
      end
  ...
  def rhc_restore(app)
    ...
    rhc_do('rhc_restore') do
      time = Benchmark.realtime do 
        run("#{$rhc_client} snapshot restore #{app.name} -f '#{app.snapshot}' -l #{app.login} -p #{app.password} -d").should == 0
      end
  ...
  def rhc_tidy(app)
    rhc_do('rhc_tidy') do
      run("#{$rhc_client} app tidy #{app.name} -l #{app.login} -p #{app.password} -d").should == 0
    end
  end

  def rhc_create_app(app, use_hosts=true, misc_opts='')
    rhc_do('rhc_create_app') do
      cmd = "#{$rhc_client} app create #{app.name} #{app.type} -r #{app.repo} -l #{app.login} -p #{app.password} #{misc_opts} -d"
  ...
  def rhc_embed_add(app, type)
    rhc_do('rhc_embed_add') do
      result = nil
      time = Benchmark.realtime do 
        result = run_stdout("#{$rhc_client} cartridge add #{type} -a #{app.name} -l #{app.login} -p #{app.password} -d")
      end
  ...
  def rhc_embed_remove(app, type)
    rhc_do('rhc_embed_remove') do
      # puts app.name
      time = Benchmark.realtime do 
        run("#{$rhc_client} cartridge remove #{type} -a #{app.name} -l #{app.login} -p #{app.password} -d").should == 0
      end
  ...
  def rhc_ctl_stop(app)
    rhc_do('rhc_ctl_stop') do
      time = Benchmark.realtime do 
        run("#{$rhc_client} app stop #{app.name} -l #{app.login} -p #{app.password} -d").should == 0
      end
      log_event "#{time} STOP_APP #{app.name} #{app.login}"
      time = Benchmark.realtime do 
        run("#{$rhc_client} app show --state #{app.name} -l #{app.login} -p #{app.password}  | grep '#{app.get_stop_string}'").should == 0
      end
  ...
  def rhc_add_alias(app)
    rhc_do('rhc_add_alias') do
      time = Benchmark.realtime do 
        run("#{$rhc_client} alias add #{app.name} '#{app.name}-#{app.namespace}.#{$alias_domain}' -l #{app.login} -p #{app.password} -d").should == 0
      end
  ...
  def rhc_remove_alias(app)
    rhc_do('rhc_remove_alias') do
      time = Benchmark.realtime do 
        run("#{$rhc_client} alias remove #{app.name} '#{app.name}-#{app.namespace}.#{$alias_domain}' -l #{app.login} -p #{app.password} -d").should == 0
      end
  ...
  def rhc_ctl_start(app)
    rhc_do('rhc_ctl_start') do
      time = Benchmark.realtime do 
        run("#{$rhc_client} app start #{app.name} -l #{app.login} -p #{app.password} -d").should == 0
      end
      log_event "#{time} START_APP #{app.name} #{app.login}"
      time = Benchmark.realtime do 
        run("#{$rhc_client} app show --state #{app.name} -l #{app.login} -p #{app.password} | grep '#{app.get_stop_string}'").should == 1
      end
      log_event "#{time} STOP_APP #{app.name} #{app.login}"
    end
  end
  ...
  def rhc_ctl_restart(app)
    rhc_do('rhc_ctl_restart') do
      time = Benchmark.realtime do 
        run("#{$rhc_client} app restart #{app.name} -l #{app.login} -p #{app.password} -d").should == 0
      end
  ...
  def rhc_ctl_destroy(app, use_hosts=true)
    rhc_do('rhc_ctl_destroy') do
      time = Benchmark.realtime do 
        run("#{$rhc_client} app delete #{app.name} -l #{app.login} -p #{app.password} --confirm -d").should == 0
      end
      log_event "#{time} DESTROY_APP #{app.name} #{app.login}"
      time = Benchmark.realtime do 
        run("#{$rhc_client} app show --state #{app.name} -l #{app.login} -p #{app.password} | grep 'does not exist'").should == 0
      end
      log_event "#{time} STATUS_APP #{app.name} #{app.login}"
